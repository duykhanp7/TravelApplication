...
import 'package:travel_booking_tour/features/explore/blocs/bloc_see_more_screen.dart';
  ...
  Widget _buildBody() {
    return BlocBuilder<BlocSeeMoreScreen, BlocSeeMoreState>(
      buildWhen: (previous, current) =>
          current is BlocSeeMoreStateLoadingMoreSuccess ||
          current is BlocSeeMoreStateLoadingMoreFail ||
          current is BlocSeeMoreStateLoadingMore,
      builder: (context, state) {
        if (state is BlocSeeMoreStateLoadingMoreSuccess) {
          isLoading = false;
          mainData.addAll(state.data?.toList() ?? []);
        } else if (state is BlocSeeMoreStateLoadingMoreFail) {
          isLoading = false;
        }
            ...
            child: Stack(
              children: [
                ...
                Positioned(
                    ...
                    bottom: 20,
                    child: Visibility(
                        visible: state is BlocSeeMoreStateLoadingMore,
                        child: Container(
                          height: 50,
